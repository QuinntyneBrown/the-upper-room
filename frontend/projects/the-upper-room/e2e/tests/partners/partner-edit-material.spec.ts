// traces_to: L2-037
import { test, expect } from '@playwright/test';

const existingPartner = {
  id: 'p1',
  name: 'Grace Church',
  website: 'https://grace.org',
  cityId: 'Toronto',
  contactCount: 3,
  tags: [],
  archived: false,
  logo: null,
  descriptionMarkdown: null,
  addresses: [],
  socialLinks: [],
};

async function seedAndNavigate(page: import('@playwright/test').Page): Promise<void> {
  await page.route('**/api/v1/partners/p1', (route) => {
    void route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(existingPartner) });
  });
  await page.goto('/sign-in');
  await page.evaluate(() => {
    const win = window as unknown as {
      __setTestToken?: (t: string) => void;
      __setRbac?: (s: { roles: string[]; permissions: string[] }) => void;
    };
    win.__setTestToken?.('lead-token');
    win.__setRbac?.({ roles: ['CityLead'], permissions: ['Partner:Create'] });
  });
  await page.route('/api/v1/notifications**', (r) =>
    r.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ items: [], total: 0 }) }),
  );
  await page.goto('/partners/p1/edit');
}

test('name input is wrapped in a Material form field', async ({ page }) => {
  await seedAndNavigate(page);
  const field = page.locator('mat-form-field').filter({ has: page.getByTestId('partner-name') });
  await expect(field).toBeVisible();
});

test('save button is a Material flat button', async ({ page }) => {
  await seedAndNavigate(page);
  await expect(page.getByTestId('partner-submit')).toHaveAttribute('mat-flat-button');
});

test('cancel button is a Material stroked button', async ({ page }) => {
  await seedAndNavigate(page);
  await expect(page.getByTestId('partner-cancel')).toHaveAttribute('mat-stroked-button');
});

test('social link platform renders as Material select', async ({ page }) => {
  await seedAndNavigate(page);
  await page.getByTestId('social-add-button').click();
  const field = page.locator('mat-form-field').filter({ has: page.locator('mat-select').first() });
  await expect(field).toBeVisible();
});

test('remove social link button is a Material icon button', async ({ page }) => {
  await seedAndNavigate(page);
  await page.getByTestId('social-add-button').click();
  await expect(page.getByTestId('social-remove-0')).toHaveAttribute('mat-icon-button');
});
