...
The backend test suite grew from 167 to **347 tests** during the remediation — a 108% increase over the baseline:

- **Domain.Tests**:       71 (was 9)
- **Infrastructure.Tests**: 9 (was 3)
- **Application.Tests**:  160 (was 50)
- **Api.Tests**:          107 (was 105)

The 180 new tests split as:
...
- **Pure-helper unit tests** (86): `ContactsDisplayName.Build` (Contacts), `ContactsMapping.ToContact` (Contacts), `EventsMapping.ToDto` (Events), `CityScope.ForCity` / `VisibleOrNull` (Cities), `NotificationMapping.Render` / `ToDto` (Notifications), `NotesSanitizer.Instance` allow-list + `ToDto` (Notes), `RoleCatalog.HasPermission` / `PermissionsFor` / `All` shape (Domain.Rbac), `Permission` record value-equality + ToString format (Domain.Rbac), `PasswordPolicy.Evaluate` (Domain.Auth) — strong/common/compromised/identifier/character-class branches, `NotificationCatalog` shape + `Require` (Domain.Notifications), and the three Domain value objects `EmailAddress` / `PhoneNumber` / `Address` — construction guards, format validation, and lat/lon bounds.
