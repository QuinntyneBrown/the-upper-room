...
- [ ] **4.10** Replace handler-level role-string comparisons (`user.Role != Roles.SystemAdmin`) with permission checks (`permissions.Require("Contacts.Edit")`). Sweep handlers feature-by-feature. _2026-05-10: in-flight. Done: `Audit/ListAuditEntriesHandler` (`Audit:Read`); `Contacts/ListContactsHandler` + `Contacts/GetContactHandler` (`City:Switch`); `Ideas/IdeasController.ChangeStatus` (`Idea:Update`); `Partners/PartnersController` 4 sites + List filter (`City:Switch`). Remaining: `Contacts/ContactsController` static helpers (`StoreCount`/`Search`) — deferred until Dashboard + Search controllers are migrated; they're purely static and called from those features._
