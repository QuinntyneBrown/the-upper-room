...
- [ ] **4.10** Replace handler-level role-string comparisons (`user.Role != Roles.SystemAdmin`) with permission checks (`permissions.Require("Contacts.Edit")`). Sweep handlers feature-by-feature. _2026-05-10: in-flight. Done so far: `Audit/ListAuditEntriesHandler` (now uses `IPermissionChecker.HasPermission(role, "Audit", "Read")`). Remaining: `Contacts/ContactsController`, `Contacts/GetContactHandler`, `Contacts/ListContactsHandler`, `Ideas/IdeasController`, `Partners/PartnersController` (4 sites)._
