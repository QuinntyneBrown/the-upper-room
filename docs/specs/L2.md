...
## L2-073a: S.O.L.I.D. Design Principles
**Traces to:** L1-022

All backend production code (`backend/src/**`) must adhere to the S.O.L.I.D. design principles:

- **Single Responsibility**: Each class has one reason to change. MediatR handlers, validators, and policies are one-per-concern; no "manager" or "service" classes that bundle unrelated operations.
- **Open/Closed**: Behavior is extended by adding new types (handlers, behaviors, policies, validators), not by editing existing ones. New cross-cutting behavior is added as a new MediatR `IPipelineBehavior`, not by inserting branches into existing behaviors.
- **Liskov Substitution**: Derived types must be safely substitutable for their base. No subclass may throw `NotSupportedException` for inherited members or strengthen preconditions.
- **Interface Segregation**: Public abstractions are narrow and role-specific (e.g., `IContactReader` and `IContactWriter` rather than a fat `IContactService`). Consumers depend only on the methods they call.
- **Dependency Inversion**: High-level modules (Application) depend only on abstractions; concrete implementations live in Infrastructure and are wired via `Microsoft.Extensions.DependencyInjection`. The Domain project has zero dependencies on Application, Infrastructure, or Api. The Application project has zero dependencies on Infrastructure or Api.

**Acceptance Criteria:**
1. Given a static analysis pass with `NetArchTest` (or equivalent) in `TheUpperRoom.Architecture.Tests`, when run, then it asserts: `Domain` references neither `Application`, `Infrastructure`, nor `Api`; `Application` references neither `Infrastructure` nor `Api`; `Api` does not reference `Infrastructure` types directly except via DI registration extension methods.
2. Given any class in `Application` or `Infrastructure`, when inspected, then it has no more than one public responsibility surface (e.g., a single MediatR `Handle` method, a single `Validate` method, a single `IPipelineBehavior.Handle`); "service" classes containing more than one unrelated public method must be refactored.
3. Given any new cross-cutting concern (logging, validation, authorization, transactions, caching), when added, then it is implemented as a new `IPipelineBehavior<TRequest, TResponse>` registered in the DI container, not as inline code in an existing handler or behavior.
4. Given any abstraction defined in `Application`, when reviewed, then it exposes only the operations its callers use; "fat" interfaces (>5 methods covering unrelated read/write/admin concerns) are forbidden.

---

## L2-073b: Microsoft.Extensions.* as Standard Primitives
**Traces to:** L1-022

The backend must use the Microsoft.Extensions.* libraries as the sole primitives for the listed cross-cutting concerns. No bespoke or third-party replacement may be introduced for any of them.

| Concern | Required library | Required usage |
| --- | --- | --- |
| Logging | `Microsoft.Extensions.Logging.Abstractions` | Inject `ILogger<T>` into every class that logs. Use structured logging with named placeholders (`_logger.LogInformation("Contact {ContactId} archived by {UserId}", id, userId);`). String concatenation/interpolation in log messages is forbidden. Provider: Serilog via `Serilog.Extensions.Logging` is permitted, but consumers must depend only on `ILogger<T>`. |
| Configuration | `Microsoft.Extensions.Configuration` | All configuration is read via `IConfiguration` populated by `appsettings.json`, `appsettings.{Environment}.json`, environment variables, and (in Development) user secrets, in that order. No `ConfigurationManager`, no static config classes, no direct `Environment.GetEnvironmentVariable` outside `Program.cs` bootstrap. |
| Dependency Injection | `Microsoft.Extensions.DependencyInjection` | The built-in `IServiceCollection`/`IServiceProvider` is the only IoC container. No Autofac, Lamar, SimpleInjector, or service locator pattern. Each project exposes a single `AddXxx(this IServiceCollection)` extension in a `DependencyInjection.cs` file (e.g., `AddApplication`, `AddInfrastructure`). |
| Options | `Microsoft.Extensions.Options` | All settings sections bind to strongly-typed POCOs registered via `services.AddOptions<TOptions>().Bind(config.GetSection("X")).ValidateDataAnnotations().ValidateOnStart()`. Consumers inject `IOptions<T>`, `IOptionsSnapshot<T>` (per-request), or `IOptionsMonitor<T>` (singletons reacting to change). Reading settings via raw `IConfiguration["..."]` outside the bootstrap is forbidden. |
| Hosting | `Microsoft.Extensions.Hosting` | The generic host (`Host.CreateApplicationBuilder` / `WebApplication.CreateBuilder`) hosts the API. Long-running work is implemented as `BackgroundService`/`IHostedService`. |
| HTTP | `Microsoft.Extensions.Http` | All outbound HTTP calls go through `IHttpClientFactory` via `services.AddHttpClient<TTypedClient>()`. Manual `new HttpClient()` instantiation is forbidden outside tests. |

**Acceptance Criteria:**
1. Given a grep across `backend/src/**`, when searching for `new HttpClient(`, `Console.WriteLine(`, `Trace.Write`, `ConfigurationManager`, or `Environment.GetEnvironmentVariable(` outside `Program.cs`, then zero matches are found.
2. Given any class that emits logs, when inspected, then it depends on `ILogger<T>` (constructor-injected) and uses structured logging placeholders; failing the placeholder check fails the build via a Roslyn analyzer (e.g., `SerilogAnalyzer` or `Microsoft.Extensions.Logging.Abstractions` analyzer rule `CA1848`/`CA2254`).
3. Given any settings section in `appsettings.json`, when bound, then there is a corresponding `TOptions` POCO registered with `ValidateDataAnnotations().ValidateOnStart()`, and the application fails fast at startup if validation fails.
4. Given any project in `backend/src/**`, when its `csproj` is inspected, then it references no third-party DI container, logging framework abstraction, or configuration framework other than the approved Microsoft.Extensions.* and Serilog (provider-only) packages.
5. Given the `TheUpperRoom.Application` and `TheUpperRoom.Infrastructure` projects, when inspected, then each exposes exactly one public DI registration extension method (`AddApplication` / `AddInfrastructure`) on `IServiceCollection`, and `Program.cs` calls only those extensions plus framework-provided ones.
